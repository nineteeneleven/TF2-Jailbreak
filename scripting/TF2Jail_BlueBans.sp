...
public DB_Callback_Connect(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	else
	{
		...
		Format(sQuery, sizeof(sQuery), "CREATE TABLE IF NOT EXISTS test (`id` INT NOT NULL AUTO_INCREMENT, `timestamp` INT, `offender_steamid` VARCHAR(22), `offender_name` VARCHAR(32), `admin_steamid` VARCHAR(22), `admin_name` VARCHAR(32), `bantime` INT(16), `timeleft` INT(16), `reason` VARCHAR(200), PRIMARY KEY (id));", g_sLogTableName);
		SQL_TQuery(gH_BanDatabase, DB_Callback_Create, sQuery);
	}
}
